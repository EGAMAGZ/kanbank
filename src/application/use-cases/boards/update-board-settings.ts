import { type UpdateBoardInput, UpdateBoardSchema } from "../../dto/board.dto.js";
import { type Id } from "../../../shared/types/index.js";
import { ValidationError, EntityNotFoundError } from "../../../domain/errors/domain-errors.js";
...
export class UpdateBoardSettingsUseCase {
  ...
  async execute(id: Id<"Board">, input: UpdateBoardInput): Promise<void> {
    ...
    if (parsed.data.autoCloseDays !== undefined) changes.autoCloseDays = parsed.data.autoCloseDays;
    if (parsed.data.autoCloseEnabled !== undefined) changes.autoCloseEnabled = parsed.data.autoCloseEnabled;
    if (parsed.data.publicLink !== undefined) changes.publicLink = parsed.data.publicLink;
    if (parsed.data.accessControl !== undefined) changes.accessControl = parsed.data.accessControl;
